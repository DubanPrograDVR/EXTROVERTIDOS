...
} from "./utils/publicationModalUtils";
...
export default function PublicationModal({
  ...
  useEffect(() => {
    ...
    if (publication) {
      setEditData({
        ...
        imagenes: getRealPublicationImages(publication.imagenes),
  ...
  const validImagesLength = canManageAdminMedia
    ? getRealPublicationImages(editData.imagenes).length
    : Math.max(publication?.imagenes?.length || 0, 1);

  useEffect(() => {
    setCurrentImageIndex((prev) =>
      validImagesLength > 0 ? Math.min(prev, validImagesLength - 1) : 0,
    );
  }, [validImagesLength]);
  ...
  const handleSaveChanges = async () => {
    const realExistingImages = getRealPublicationImages(editData.imagenes);
    ...
    try {
      ...
      if (cleanedData.tipo_entrada !== "pagado") {
        cleanedData.precio = null;
      }
      if (cleanedData.tipo_entrada !== "venta_externa") {
        cleanedData.url_venta = null;
      }
      ...
      // Capa defensiva: re-aplicar word-wrap (76 chars) sobre descripcion y
      // mensajes de marketing antes de persistir, por si el valor entró por
      // una vía no controlada (paste, hidratación, datos legados).
      const dataToPersist = wrapPersistedFields(cleanedData);

      await updateEvent(publication.id, dataToPersist, { adminOverride: true });
  ...
  const modalClassName = [
    ...
    isEditMode ? "publication-modal--editing" : "",
  ]
    .filter(Boolean)
    .join(" ");
              ...
              {(hasDescription || isEditMode) && (
                <AccordionSection
                  ...
                  <div className="publication-modal__description-content rich-text">
                    {!isEditMode &&
                      renderRichText(descripcion, { keyPrefix: "pub-desc" })}
                    {isEditMode && (
                      <WrappingTextarea
              ...
              (instagram ||
                ...
                <div className="publication-modal__social-section">
                  ...
                  <div className="publication-modal__social-bar">
                    ...
                    {youtube && (
                      <a
                        href={buildSocialUrl(youtube, "https://youtube.com/")}
                        target="_blank"
                        rel="noopener noreferrer"
